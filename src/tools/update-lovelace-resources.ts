...
import { RacelandshopLogger } from "./racelandshop-logger";
...
export async function updateLovelaceResources(
  ...
  const logger = new RacelandshopLogger("updateLovelaceResources");
  const resources = await fetchResources(hass);
  const namespace = `/racelandshopfiles/${repository.full_name.split("/")[1]}`;
